...
```{r}
gf_point(price ~ distance, data = ride, size = 4, alpha = .15) %>%
...
```{r}
coef(ride_lm_median)
summary(ride_lm_median)$coefficients[,2]
CI_b0 <- 17.013775 + c(-1, 1) * 1.96 * 0.1756361
CI_b1 <- 2.889949 + c(-1, 1) * 1.96 * 0.1533785
CI_b0
CI_b1
```

The most common Confidence Interval in social sciences is at 95%, in other words, if we take repeated samples and construct the confidence interval for each sample, 95% of the intervals will contain the true unknown value of the parameter [@james2021statistical].

The interpretation of both CI are as follow:

- In the absence of any x value, the price will be, on average, somewhere between $16.67 and 17.36 (b0).
- With 95% of certainty, a 1-mile increase in distance is associated with a price increase from $2.58 to 3.19 (b1).

```{r}
CI_b0_99 <- 17.013775 + c(-1, 1) * 2.58 * 0.1756361
CI_b1_99 <- 2.889949 + c(-1, 1) * 2.58 * 0.1533785
CI_b0_99
CI_b1_99
```
...
$$
In both cases, the alternative hypothesis is that the parameters are different from zero.

```{r}
t_b0 = (17.013775 - 0) / 0.1756361
t_b1 = (2.889949 - 0) / 0.1533785
t_b0
t_b1
```

Considering the test statistic results, in both cases the null hypothesis can be rejected. In particular, the test statistic of the estimated b0 (96.87) falls far away of the interval constructed with 95% of confidence; on the other hand, the test statistic of the estimated b1 (18.84) also falls out of the interval constructed at 95% of confidence.

In addition, p-values support this interpretation of uncertainty in the estimation of parameters b0 and b1. According to the summary of the model, probably the B0 and B1 parameters are not equal zero in the population. Finally, based on the CI, test statistic and p-values, there is some relationship between price and distance. 

```{r}
summary(ride_lm_median)
```
...
The results of the median-centering model are useful to explain part of the variability of prices using the median of distance. In practice, it allows to estimate the average costs of rides and their increase based on distance.

Passengers or user of Uber and/or Lyft in Boston could use this information to estimate the cost of their rides using the median of distance (2.275 miles) as a proxy of the average price they could pay.
