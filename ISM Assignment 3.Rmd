...
Due to the use of mean-centered predictors, the interpretation of the coefficients is fairly straightforward. Indeed, the interpretation of the intercept (B0) is the same as before: the average fertility rate among countries classified as Low per capita income is 3.83, when all other predictors are zero.
In turn, the estimated parameters B1 and B2 represent the variation that one unit of years of education or percentage of contraceptive use has on the fertility rate holding all other predictors constant (among them and with respect to GNI categories). In this case, independent of the reference category, the effects of B1 and B2 on the fertility rate are negative and significant at the 1% significance level; especially the variable years of education (`educ_fem_mc`), where for one unit of variation from the mean (increase), the fertility rate decreases, on average, by .124 units (holding all other attributes constant [both contraceptive use and the respective GNI category]).
