...
To answer the question, the interaction term (b3) is the average difference between the slope of those countries where the average female education is higher than 6 years compared to those with lower female education levels. In this case, given that the contraceptive use variable is centered on the mean, the interaction does not add any effect to the base model (recall that when calculating the adjusted means for the category of more than 6 years of female schooling we would have: (3.328347 - 1.304661) + ((-0.049391 + 0.035649 )*0) = 2.02 .
However, as the values of contraceptive use increase with respect to their mean, we can assume that their effect on the fertility rate among those countries with more than 6 years of average female education will also increase.

This can be checked using the lsmeans function.

```{r}
lsmeans(m5, ~ educ_fem_cat)
```
