...
m7 = lm(weight_lbs ~ ott_mean * midwest, data = dat)
m8 = lm(weight_lbs ~ ott_mean + northeast, data = dat)
m9 = lm(weight_lbs ~ ott_mean * northeast, data = dat)
m10 = lm(weight_lbs ~ ott_mean + west, data = dat)
m11 = lm(weight_lbs ~ ott_mean * west, data = dat)
m12 = lm(weight_lbs ~ ott_mean + south, data = dat)
m13 = lm(weight_lbs ~ ott_mean * south, data = dat)
```

anova()

df_stats(weight_lbs ~ us_region, data = dat, mean, length)


```{r}
dat %>%
  group_by(us_region) %>%
  summarise(N = n(), Mean = mean(weight_lbs, na.rm=TRUE))
```
