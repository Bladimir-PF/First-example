...
Based on these results, it was stated that the relationship is positive and roughly linear (r=.62), with departure of data at the lower and upper extremes (red line). From the summary of the simple linear regression, the costs of Room and Board (RB) is, on average, -129.75 (b0) when the Early Career Pay (ECP) equals zero. The slope (b1) is 0.216, and it can be interpreted as for a one-unit increase in ECP (in USD), the cost of RB increases, on average, by 
...
The partial slope of Early Career Pay (b1) is 0.2039532, not much different from the value obtained previously (b0 = 0.2161361), which means that for one unit change (in dollars) in costs of Early Career Pay, the costs of Room and Board increase, on average and holding the membership constant (Type of institution), by 0.20 dollars. Again, In this case, the slope is small due to the metric of this variable, whose values are expressed in 1.000 dollar units. A more practical interpretation would be that the costs of RB increase, on average, by 203,95 dollars per 1.000 units (dollars) increase in ECP.

The partial slope of Type of institution (b2) corresponds to Private institutions category, and its interpretation will be as follow: the costs of RB increase, on average, by 1.010,98 (b0+b1) when students enrolled in Private institutions. The raw b2 (1230.49) reports the difference in average RB costs between Public and Private institutions.
...
R-squared and RSE are common indicators of fitting of the model to data. In this case, we have the following changes between the first and second model:
...
The r-squared and RSE improved. The predictors included in the multiple regression model (Early Career Pay and Type of institution) explains around 43,6% of the variability of RB costs, a slight increase compared to the previous one (38,9%). On the other hand, the sigma decreased from 2160.76 to 2077.07; this means that the difference between the observed RB costs and predicted ones is, on average, 2077.07.

In addition, due to the models are nested, we can analyze the variance decomposition to support or challenge the our results. The objective is provide more evidence that the added predictor (Type of institution) helps to explain variation over and above the first model (simple one).

```{r}
anova(rb_lm, rb_mlm)
```

The large F-statistics (61.425) suggests that the Type of institution is related to RB costs.
