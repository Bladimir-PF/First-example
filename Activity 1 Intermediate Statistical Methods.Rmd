...
```{r}
10.44 + 2.89 * 0.030 #min
10.44 + 2.89 * 1.300 #1st quart
10.44 + 2.89 * 2.275 #median
10.44 + 2.89 * 2.216 #mean
10.44 + 2.89 * 2.860 #3rd quart
10.44 + 2.89 * 7.460 #max
```

Again, the linear regression shows that as the distance increases so does the price.
...
First, the r-squared equals 12,4%, that is, the distance just explains, approximately, the 12% of the price variability. On the other hand, the standard deviation of the observed prices (sigma) respect with the regression line is 8.77.
...
These results make more sense in the context of the problem. First, the r-squared looks small considering, first, the size of the sample and, second, the kind of relationship expected between price and distance. When we analyze something like the price of rides, it is normal to assume that distance is one of the main factors involved. Therefore, a higher r-square it was expected.

```{r, echo=FALSE, results='hide'}
8.78/16.8
```

In addition, the sigma estimates expresses the high dispersion observed in the first plot between price and distance. The difference between the observed prices and predicted ones is large (8.77), which means that the percentage error is 52.3%. Once more, the variability is higher than expected and opens the question about what variable can aid to improve the linear regression estimation.
